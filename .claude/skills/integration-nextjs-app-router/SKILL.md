...
This project intentionally uses anonymous-only, privacy-safe PostHog analytics.
Do not add `posthog.identify()`, Supabase auth listeners for analytics identity,
automatic exception capture, session replay, autocapture, or any event properties
outside the allowlist in `lib/analytics.ts`. Track only manual page views and the
explicit product events already defined for the app.
